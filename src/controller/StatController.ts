...
export class StatController extends BaseController {
  ...
  async playerMinutesStat(
    ...
    if (teamId) {
      ...
      return response.status(400).send({ name: 'search_error', message: `Team id required field` });
    }
  }
  ...
  async summaryScoringForTeam(
    ...
    if (competitionId && teamId && divisionId) {
      ...
      return response
        .status(400)
        .send({ name: 'search_error', message: `Required fields not filled` });
    }
  }
  ...
  async scoringStatsByMatchForTeam(
    ...
    if (competitionId && teamId && divisionId) {
      ...
      return this.teamService.scoringStatsByMatch(
        competitionId,
        teamId,
        matchId,
        divisionId,
  ...
  async scoringStatsByPlayer(
    ...
    if (!competitionId) {
      return response
        .status(400)
        .send({ name: 'search_error', message: `CompetitionId data is missing` });
    }
    if (playerId && !divisionId) {
      return response
        .status(400)
        .send({ name: 'search_error', message: `DivisionId data is missing` });
    }
    ...
    if (
      isNotNullAndUndefined(offset) &&
      isNotNullAndUndefined(limit) &&
      isArrayPopulated(getScoringData.count)
    ) {
      return {
        ...paginationData(stringTONumber(getScoringData.count[0]['totalCount']), limit, offset),
        result: getScoringData.finalData,
      };
    } else {
  ...
  async exportTeamAttendance(
    ...
    if (isArrayPopulated(gameTimeData)) {
      gameTimeData.map(e => {
        ...
        e['Play %'] =
          e.playTimeTeamMatches == 0 || e.playTimeTeamMatches == null
            ? ''
    ...
    fastcsv
      .write(gameTimeData, { headers: true })
      .on('finish', function () {})
      .pipe(response);
  }
  ...
  async exportScoringStatsByPlayer(
    ...
    if (!competitionId) {
      return response
        .status(400)
        .send({ name: 'missing_data', message: `Data missing for field competitionId` });
    }
    if (playerId && !divisionId) {
      return response
        .status(400)
        .send({ name: 'missing_data', message: `Data missing for field divisionId` });
    }
    ...
    response.setHeader('content-type', 'text/csv');
    fastcsv
      .write(playerScoreData, { headers: true })
      .on('finish', function () {})
      .pipe(response);
  }
  ...
  async borrowsForPlayer(
    ...
    if (playerId) {
      ...
      return response
        ...
        .send({
          name: 'search_error',
          message: `CompetitionId, teamId & playerId are mandatory fields`,
        });
    }
  }
  ...
  async positionTracking(
    ...
    if (isNotNullAndUndefined(competitionId)) {
      ...
      return response
        .status(400)
        .send({ name: 'search_error', message: `Missing required parameters` });
    }
  }
  ...
  async positionTrackingExport(
    ...
    if (isNotNullAndUndefined(competitionId)) {
      ...
      if (!isNotNullAndUndefined(gameTimeAttendanceData) || gameTimeAttendanceData.length == 0) {
      ...
      response.setHeader('content-type', 'text/csv');
      fastcsv
        .write(gameTimeAttendanceData, { headers: true })
        .on('finish', function () {})
        .pipe(response);
    } else {
      return response
