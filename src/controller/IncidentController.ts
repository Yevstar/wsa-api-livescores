...
export class IncidentController extends BaseController {
  ...
  async find(
    ...
    if (
      isNotNullAndUndefined(incidentId) ||
  ...
  async loadIncidents(
    ...
    return this.incidentService.findIncidentsByParam(
      matchId,
      competitionId,
      teamId,
      playerId,
  ...
  async addIncident(
    ...
    return await this.addOrEditIncident(false, playerIds, incident, response);
  }
  ...
  private async addOrEditIncident(
    ...
    try {
      if (incident) {
        ...
        return response
          .status(400)
          .send({ name: 'validation_error', message: `Incident can not be null` });
      }
    } catch (e) {
  ...
  async editIncident(
    ...
    return await this.addOrEditIncident(true, playerIds, incident, response);
  }
  ...
  async addIncidentMedia(
    ...
    return await this.uploadOrRemoveIncidentMedia(user, incidentId, guid, files, [], response);
  }
  ...
  private async uploadOrRemoveIncidentMedia(
    ...
    try {
      ...
      if (incidentMediaIds && incidentMediaIds.length >= 0) {
        ...
        removedIncidentMediaArray = (
          await this.incidentService.fetchIncidentMedia(incidentId, guid)
      ...
      if (removedIncidentMediaArray && removedIncidentMediaArray.length > 0) {
        ...
        removedIncidentMediaArray.forEach(async incidentMedia => {
          if (incidentMedia.mediaUrl) {
            ...
            mediaRemovalPromises.push(this.incidentService.removeIncidentMedia(incidentMedia));
          }
        });
      ...
      if (files && files.length > 0) {
        ...
        for (const file of files) {
          ...
          if (upload) {
            ...
            media.push(
              this.incidentService.createIncidentMedia(
                incidentId,
                guid,
                user.id,
                url,
  ...
  private async updateMediaIncidentId(incident: Incident) {
    let incidentMediaList = await this.incidentService.fetchIncidentMedia(
      incident.id,
      incident.guid,
    );
  ...
  async exportIncidents(
    ...
    if (
      ...
      fastcsv
        .write(incidentData.results, { headers: true })
        .on('finish', function () {})
        .pipe(response);
    } else {
      return response.status(400).send({
