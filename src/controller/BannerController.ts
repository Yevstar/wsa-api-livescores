...
export class BannerController extends BaseController {
  ...
  async find(
    ...
    return await this.bannerService.findByParams(
      organisationId,
      competitionIds,
      /* pageType, */ format,
    );
    ...
    try {
      ...
      if (file) {
        if (isPhoto(file.mimetype)) {
          let filename = `/media/banner/${organisationId}_${competitionId}_${timestamp()}.${fileExt(
            file.originalname,
          )}`;
        ...
        for (const i in files) {
          if (files[i]) {
            if (isPhoto(files[i].mimetype)) {
              let filename = `/media/banner/${
                imageTypes[i]
              }_${organisationId}_${timestamp()}.${fileExt(files[i].originalname)}`;
    ...
    if (isNotNullAndUndefined(banner)) {
      if (banner.bannerUrl) {
        const fileName = await this.firebaseService.getFileNameFromUrl(
          JSON.stringify(banner.bannerUrl),
          'banner%2F',
        );
      ...
      if (banner.horizontalBannerUrl) {
        const fileName = await this.firebaseService.getFileNameFromUrl(
          JSON.stringify(banner.horizontalBannerUrl),
          'banner%2F',
        );
        await this.firebaseService.removeMedia(fileName);
      }
      if (banner.squareBannerUrl) {
        const fileName = await this.firebaseService.getFileNameFromUrl(
          JSON.stringify(banner.squareBannerUrl),
          'banner%2F',
        );
        await this.firebaseService.removeMedia(fileName);
      }
  ...
  async deleteImageById(
    ...
    if (isNotNullAndUndefined(banner) && ratioType) {
      const fileName = await this.firebaseService.getFileNameFromUrl(
        JSON.stringify(banner[`${ratioType}BannerUrl`]),
        'banner%2F',
      );
      await this.firebaseService.removeMedia(fileName);
