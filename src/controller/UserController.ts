...
export class UserController extends BaseController {
  ...
  async loginWithFacebook(
    ...
    try {
      ...
      if (tokenResponse.status == 200) {
        ...
        return await this.responseWithTokenAndUser(
  ...
  async loginWithGoogle(
    ...
    try {
      ...
      return response
        .status(404)
        .send({ name: 'validation_error', message: 'Invalid access token' });
    }
  }
  ...
  private async loadUserWatchlist(
    ...
    let competitionOrganisationIds = watchlist
      .filter(item => item.entityTypeId == EntityType.COMPETITION_ORGANISATION)
      .map(item => item.entityId);
    let teamIds = watchlist
      .filter(item => item.entityTypeId == EntityType.TEAM)
      .map(item => item.entityId);
  ...
  private async checkUserWatchlist(userData: User) {
    let deviceIds = (await this.deviceService.getUserDevices(userData.id)).map(
      device => device.deviceId,
    );
    if (isArrayPopulated(deviceIds)) {
      deviceIds.forEach(deviceId => {
        this.evaluateUserWatchlist(userData, deviceId, true);
      });
    }
  }
  ...
  async addUmpire(
    ...
    if (isUmpire && isUmpireCoach) {
      ...
      for (const ure of existsTeamUres) {
        await this.deleteRolesNecessary(
          'UMPIRE_TEAM',
          foundUser,
          ure.entityId,
          EntityType.TEAM,
  ...
  async add(
    ...
    try {
      ...
      this.processSendMail(type, userData, password, entityTypeId, compId, user);
  ...
  private async processSendMail(
    ...
    if (
      this.canSendMailForAdd(type, userData) &&
    ...
    if (roleIdToDelete && linkedEntityTypeId) {
      ...
      if (isArrayPopulated(rosters)) {
        rosters.forEach(roster => {
          promiseList.push(this.rosterService.deleteById(roster.id));
        });
      }
      promiseList.push(
  ...
  private async sendUserRosterUpdateNotification(user: User) {
    if (user.id) {
      let tokens = (await this.deviceService.getUserDevices(user.id)).map(
        device => device.deviceId,
      );
      ...
      if (!infoMisMatchArray.includes(i['Email'].toLowerCase())) {
        ...
        if (teamRequired) {
          ...
          for (let org of orgArray) {
            const linkedCompetitionOrganisationArray: LinkedCompetitionOrganisation[] = await this.linkedCompetitionOrganisationService.findByNameAndCompetitionId(
              org,
              competitionId,
            );
        ...
        if (isArrayPopulated(teamDetailArray)) {
          for (let i of teamDetailArray) {
            ...
            if (teamChatRequired) {
              teamChatPromiseArray.push(this.addUserToTeamChat(i.id, userDetails));
            }
          }
          ...
          for (let org of linkedOrgDetailArray) {
            if (roleId == Role.UMPIRE) {
              ...
              if (
                (isNullOrEmpty(umpire) && isNullOrEmpty(umpireCoach)) ||
                ...
                if (!isNullOrEmpty(umpire) && (umpire == 'yes' || umpire == 'true')) {
                  createUmpireURE = true;
                }
                if (
                  !isNullOrEmpty(umpireCoach) &&
        ...
        message[`Line ${i.line}`].message.push(
          `Email "${i['Email'].toLowerCase()}" already exists with different details.`,
        );
      }
    }
  ...
  private async createUserWatchlist(
    ...
    await this.watchlistService.save(
    ...
    if (user) {
      ...
      if (competitionOrganisationIds) {
        const orgList = await this.linkedCompetitionOrganisationService.findByIds(
          competitionOrganisationIds,
        );
        orgList.forEach(org => {
  ...
  private async removeUserWatchlist(
    ...
    if (entityTypeId) {
      ...
      if (entityTypeId == EntityType.COMPETITION_ORGANISATION && entityId != 0) {
        teamIds = (await this.teamService.teamsByCompetitionOrganisationId(entityId)).map(
          team => team.id,
        );
      }
    ...
    await this.watchlistService.deleteByParam(
      user ? user.id : undefined,
      deviceId,
      entityId,
      entityTypeId,
    ...
    if (user) {
      ...
      if (compId) {
        await this.ureService.deleteRoleByParams(
          compId,
          EntityType.COMPETITION,
          Role.SPECTATOR,
          user.id,
  ...
  private async evaluateUserWatchlist(user: User, deviceId: string, notifyAnyChange: boolean) {
    ...
    if (isArrayPopulated(ures)) {
      ures.forEach(ure => {
        if (ure.roleId == Role.MANAGER || ure.roleId == Role.PLAYER || ure.roleId == Role.COACH) {
    ...
    userWatchlist.forEach(watchItem => {
      if (
        watchItem.entityTypeId == EntityType.COMPETITION_ORGANISATION &&
    ...
    if (user && deviceId && (teamIdSet.size != 0 || competitionOrganisationIdSet.size != 0)) {
