...
export class GameAttendanceController extends BaseController {
  ...
  async find(
    ...
    return this.gameTimeAttendanceService.findByParam(
      matchId,
      teamId,
      playerId,
      period,
  ...
  async savePlayers(
    ...
    if (match.matchStatus == null && match.matchStatus != 'ENDED') {
      ...
      if (attendance.length > 0) {
        ...
        return response
          .status(400)
          .send({ name: 'save_attendance_error', message: 'List attendance is empty' });
      }
    } else {
      return response
        .status(400)
        .send({
          name: 'save_attendance_error',
          message: 'Attendance cannot be submitted after a match has ended',
        });
    }
  }
  ...
  async updatePlayers(
    ...
    if (!matchId)
      return response.status(400).send({ name: 'validation_error', message: 'Match id required' });
    if (!teamId)
      return response.status(400).send({ name: 'validation_error', message: 'Team id required' });
      ...
      if (attendance.length > 0) {
        try {
          ...
          return response.status(400).send({
            ...
            message: 'Failed to save the attendances.',
          });
        }
        ...
        return response
          .status(400)
          .send({ name: 'updated_attendance_error', message: 'List attendance is empty' });
      }
    } else {
      return response
        .status(400)
        .send({
          name: 'update_attendance_error',
          message: 'Attendance cannot be submitted after a match has ended',
        });
    }
  }
  ...
  private async updateGameTimeAttendances(
    ...
    for (let att of attendances) {
      ...
      let gta = await this.gameTimeAttendanceService.findPlayerByParamAndLast(
        match.id,
        teamId,
        att.playerId,
      );
      if (gta) {
        if (
          (att.period && gta.period != att.period) ||
    ...
    let tokens = (await this.deviceService.findScorerDeviceFromRoster(match.id)).map(
      device => device.deviceId,
    );
    if (tokens && tokens.length > 0) {
      this.firebaseService.sendMessageChunked({
  ...
  async delete(
    ...
    let result = await this.gameTimeAttendanceService.deleteByMatchAndTeam(
      matchId,
      teamId,
      period,
      false,
  ...
  async manualUpload(
    ...
    if (attendance.length > 0) {
      ...
      if (saveAttendance) {
        ...
        try {
          await this.updateGameTimeAttendances(match, teamId, attendance, null, true, user);
        } catch (err) {
          logger.error(`Failed upload of attendances due to -`, err);
          return response.status(400).send({
            err,
