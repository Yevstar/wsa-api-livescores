...
export class PlayerController extends BaseController {
  ...
  async find(
    ...
    if (isNotNullAndUndefined(competitionOrganisationId) && competitionOrganisationId != 0) {
      linkedCompetitionOrganisation = await this.linkedCompetitionOrganisationService.findById(
    ...
    if (playerInput) {
      ...
      if (file) {
        if (isPhoto(file.mimetype)) {
          let filename = `/media/${playerInput.competitionId}/${playerInput.teamId}/${
            playerInput.id
          }_${timestamp()}.${fileExt(file.originalname)}`;
  ...
  async exportCSV(
    ...
    if (isNotNullAndUndefined(competitionId) || isNotNullAndUndefined(competitionOrganisationId)) {
      ...
      fastcsv
        .write(playerData.results, { headers: true })
        .on('finish', function () {})
        .pipe(response);
    } else {
      return response.status(400).send({
  ...
  async importCSV(
    ...
    const requiredField = ['First Name', 'Last Name', 'DOB', 'Team'];
  ...
  async listTeamPlayerActivity(
    ...
    if (requestFilter.search === undefined || requestFilter.search === '')
      requestFilter.search = null;
  ...
  async findPlayers(
    ...
    if (offset !== null && offset !== undefined && limit !== null && limit !== undefined) {
      return {
        page: paginationData(playerData.matchCount, limit, offset).page,
  ...
  async exportTeamAttendance(
    ...
    response.setHeader('content-type', 'text/csv');
    fastcsv
      .write(teamAttendanceData, { headers: true })
      .on('finish', function () {})
      .pipe(response);
  }

  private async loadPlayerUser(creator: User, player: Player): Promise<any> {
    if (
      ...
      const userResults = await this.userService.findByParam(
        player.firstName,
        player.lastName,
        player.phoneNumber,
        player.dateOfBirth,
      ...
      if (userResults && userResults[0]) {
        ...
        newUser = false;
        if (
          foundUser.firstName == player.firstName &&
  ...
  async findBorrowedPlayers(
    ...
    if (lineupSelectionEnabled) {
      let lineupList = await this.lineupService.findByParams(matchId, competitionId, teamId, true);
      ...
      let gtaList = await this.gameTimeAttendanceService.findByParams(matchId, teamId, true);
      gtaList.forEach(gta => {
        if (gta.playerId) {
  ...
  @Authorized()
  @Get('/pendingInvites')
  async pendingInvites(@HeaderParam('authorization') user: User) {
  ...
  @Post('/merge')
  async merge(@Body() payload: any): Promise<any> {
