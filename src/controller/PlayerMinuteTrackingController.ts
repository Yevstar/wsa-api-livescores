...
export class PlayerMinuteTrackingController extends BaseController {
  ...
  async find(
    ...
    try {
      const trackData = await this.playerMinuteTrackingService.findByParams(
        matchId,
        teamId,
        playerId,
      );
  ...
  async record(
    ...
    try {
      ...
      let deletePMTs = [];
      const pmtIds = matchPMTData
        .filter(function (existingPMT) {
          // filter out (!) items in existing PMT which are not been
          // provided in the body
          return !(
            !!trackingData &&
            trackingData.some(function (newPMT) {
      ...
      var gtaIds = matchGTAData
        .filter(function (existingGTA) {
          // filter out items in matching deleted PMTs
          return deletePMTs.some(function (pmt) {
            return (
              existingGTA.playerId === pmt.playerId &&
      ...
      for (let i = 0; i < trackingData.length; i++) {
        if (
          isNotNullAndUndefined(trackingData[i].matchId) &&
          ...
          pmtPromises.push(this.createPMTRecord(trackingData[i], matchGTAData, user));
        }
      }
      ...
      return response
        .status(500)
        .send({ success: false, message: 'Recording tracking time failed' });
    }
  }
  ...
  private async createPMTRecord(
    ...
    try {
      if (!isNotNullAndUndefined(playerMinuteTracking.period) || playerMinuteTracking.period == 0) {
      ...
      if (matchGTAData && matchGTAData.length > 0) {
        ...
        const filteredGTA = matchGTAData.filter(
          gta =>
            gta.playerId === playerMinuteTracking.playerId &&
      ...
      if (createGTA) {
        ...
        if (player && isNotNullAndUndefined(player.teamId) && isNotNullAndUndefined(data.teamId)) {
        ...
        if (!isNotNullAndUndefined(data.id) && isNotNullAndUndefined(data.createdBy)) {
