...
export class CompetitionController extends BaseController {
  ...
  async createOrUpdate(
    ...
    if (
      stringToBoolean(competition.positionTracking) == true &&
      competition.attendanceRecordingPeriod == 'MATCH'
    ) {
    ...
    try {
      if (competition) {
        ...
        c.attendanceSelectionTime = competition.attendanceSelectionTime
          ? stringTONumber(competition.attendanceSelectionTime)
          : null;
        c.lineupMaxPlayers = competition.lineupMaxPlayers
          ? stringTONumber(competition.lineupMaxPlayers)
          : null;
        c.lineupSelectionTime = competition.lineupSelectionTime
          ? stringTONumber(competition.lineupSelectionTime)
          : null;
        ...
        if (
          isNotNullAndUndefined(competition.invitedAnyAssoc) &&
          competition.invitedAnyAssoc !== ''
        ) {
          competition.invitedAnyAssoc = JSON.parse(competition.invitedAnyAssoc);
        }

        if (
          isNotNullAndUndefined(competition.invitedAnyClub) &&
          competition.invitedAnyClub !== ''
        ) {
          competition.invitedAnyClub = JSON.parse(competition.invitedAnyClub);
        }

        const getInviteesDetail = await this.competitionInviteesService.getInviteesByCompetition(
          saved.id,
        );

        if (competition.isInvitorsChanged === 'true') {
          if (
            (isNotNullAndUndefined(competition.invitedTo) &&
              competition.invitedTo !== '' &&
              isArrayPopulated(competition.invitedTo)) ||
            (isNotNullAndUndefined(competition.invitedAnyAssoc) &&
              competition.invitedAnyAssoc !== '' &&
              isArrayPopulated(competition.invitedAnyAssoc)) ||
            (isNotNullAndUndefined(competition.invitedAnyClub) &&
              competition.invitedAnyClub !== '' &&
              isArrayPopulated(competition.invitedAnyClub))
          ) {
            ...
            if (isArrayPopulated(INVITED_TO)) {
              const getInvitationDetails = await this.competitionInviteesService.getInviteesByCompetition(
                saved.id,
              );
              ...
              if (INVITED_TO.includes(NOT_APPLICABLE)) {
                ...
                (INVITED_TO.includes(AFFILIATED_ASSOCIATION) ||
                  INVITED_TO.includes(AFFILIATED_CLUB)) &&
                ...
                const organisationTypeRefId = await this.linkedCompetitionOrganisationService.findAffiliateDetailsByOrganisationId(
                  competition.organisationId,
                );
                GET_ORGANISATIONS = await this.competitionService.getAllAffiliatedOrganisations(
                  competition.organisationId,
                  affliliateInvited,
                ...
                if (
                  INVITED_TO.includes(AFFILIATED_ASSOCIATION) &&
                  !INVITED_TO.includes(AFFILIATED_CLUB, DIRECT, ANY_ASSOCIATION, ANY_CLUB)
                ) {
                  // Association selected
                  affliliateInvited = 3;
                  ...
                  INVITED_TO.includes(AFFILIATED_CLUB) &&
                  !INVITED_TO.includes(AFFILIATED_ASSOCIATION, DIRECT, ANY_ASSOCIATION, ANY_CLUB)
                ) {
                  // Club selected
                  affliliateInvited = 4;
                ...
                if (
                  INVITED_TO.includes(DIRECT) &&
                  !INVITED_TO.includes(
                    AFFILIATED_ASSOCIATION,
                    AFFILIATED_CLUB,
                    ANY_ASSOCIATION,
                    ANY_CLUB,
                  )
                ...
                if (
                  INVITED_TO.includes(ANY_ASSOCIATION) &&
                  isNotNullAndUndefined(competition.invitedAnyAssoc) &&
                  isArrayPopulated(competition.invitedAnyAssoc)
                ) {
            ...
            if (isArrayPopulated(COMPETITION_INVITEES)) {
              const UNIQUE_COMPETITION_INVITEES = removeDuplicatesByValue(COMPETITION_INVITEES, [
                'invitedOrganisationId',
              ]);
              await this.competitionInviteesService.batchCreateOrUpdate(
                UNIQUE_COMPETITION_INVITEES,
              );
            }
            ...
            const getExistingOrganisation = await this.competitionOrganisationService.findByCompetitionId(
              saved.id,
            );
            ...
            const UNIQUE_CREATE_COMP_ORGANISATIONS = removeDuplicatesByValue(
              CREATE_COMP_ORGANISATIONS,
              ['orgId'],
            );
            ...
            for (let i of getExistingOrganisation) {
              for (let j of COMP_ORG_ARRAY) {
                if (i.orgId === j.orgId) {
                  UNIQUE_CREATE_COMP_ORGANISATIONS.splice(
                    UNIQUE_CREATE_COMP_ORGANISATIONS.findIndex(e => e.orgId === i.orgid),
                    1,
                  );
                }
              }
            ...
            if (isArrayPopulated(UNIQUE_CREATE_COMP_ORGANISATIONS)) {
              await this.competitionOrganisationService.batchCreateOrUpdate(
                UNIQUE_CREATE_COMP_ORGANISATIONS,
              );
            }
          } else {
            if (isArrayPopulated(getInviteesDetail)) {
        ...
        if (file && isPhoto(file.mimetype)) {
          ...
          if (result) {
            ...
            return response
              .status(400)
              .send({ name: 'save_error', message: AppConstants.saveLogoError });
          }
        } else {
        ...
        return response
          .status(200)
          .send({ name: 'search_error', message: AppConstants.requiredFieldsMissingError });
      }
    } catch (err) {
  ...
  async loadAdmin(
    ...
    return this.competitionService.loadAdmin(
      ...
      umpiredCompsOnly
        ? ['NAMES', 'USERS']
        : isNotNullAndUndefined(recordUmpireType)
        ? [recordUmpireType]
        : null,
      yearRefId,
      sortBy,
  ...
  ): Promise<{ linkedCompetitionOrganisations: LinkedCompetitionOrganisation[]; teams: Team[] }> {
    const linkedCompetitionOrganisations = await this.linkedCompetitionOrganisationService.findByNameAndCompetitionId(
      organisationName,
      competitionId,
    );
  ...
  async watchlist(
    ...
    linkedCompetitionOrganisations: LinkedCompetitionOrganisation[];
    teams: Team[];
    players: Player[];
  }> {
    const competitions = await this.competitionService.findByIds(competitionIds);
    const linkedCompetitionOrganisations = await this.linkedCompetitionOrganisationService.findByIds(
    ...
    try {
      if (isArrayPopulated(requestBody)) {
        ...
        for (let item of requestBody) {
          ...
          let laddeerFormDivFromDB = await this.ladderFormatDivisionService.findByLadderFormatId(
            ladderFormatSave.id,
          );

          // Ladder Format Division
          let ladderDivisionTemp = [];
          let ladderDivisionMap = new Map();
  ...
  async getCompetitions(
    ...
    if (organisationUniqueKey !== null && organisationUniqueKey !== undefined) {
      organisationId = await this.linkedCompetitionOrganisationService.findByUniqueKey(
  ...
  async loadAdminDashboard(
    ...
    return this.competitionService.loadDashboardOwnedAndParticipatingCompetitions(
      user.id,
      requestFilter,
      organisationId,
      yearRefId,
