...
import { Get, Post, Authorized, Res, QueryParam, Body, JsonController } from 'routing-controllers';
...
export class BookingController extends BaseController {
  ...
  async saveBookings(
    ...
    if (isArrayPopulated(bookings)) {
      try {
        ...
        const count = bookings.filter(
          booking =>
            !isNotNullAndUndefined(booking.userId) ||
        ...
        if (count != 0) {
          throw count == 1
            ? `${count} booking has missing information`
            : `${count} bookings has missing information`;
        }

        // Delete any existing bookings based on their user id,
        // start time and end time
        if (isNotNullAndUndefined(fromTime) && isNotNullAndUndefined(endTime)) {
          await this.deleteBookings(userId, fromTime, endTime);
        }
        ...
        for (let booking of bookings) {
          if (!isNotNullAndUndefined(fromTime) && !isNotNullAndUndefined(endTime)) {
            await this.deleteBookings(userId, booking.startTime);
          }
    ...
    } else if (isNotNullAndUndefined(fromTime) && isNotNullAndUndefined(endTime)) {
      try {
        await this.deleteBookings(userId, fromTime, endTime);
        return response.status(200).send({
          success: true,
  ...
  private async deleteBookings(userId: number, fromDate: Date, toDate: Date = undefined) {
    let existingBookings = await this.bookingService.findByParams(userId, fromDate, toDate);
