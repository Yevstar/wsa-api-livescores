...
export class MatchUmpireController extends BaseController {
  @Get('/')
  async find(@QueryParam('matchIds') matchIds: number[]): Promise<MatchUmpire[]> {
  ...
  async findByCompetition(
    ...
    const resultsFound = await this.matchUmpireService.findByCompetitionId(
      competitionId,
      requestFilter,
    );
    if (resultsFound) {
      let responseObject = paginationData(
        stringTONumber(resultsFound.countObj),
      ...
      dict.results.map(e => {
        ...
        if (isNotNullAndUndefined(e['venueStateRefId'])) {
          e['Start Time'] = convertMatchStartTimeByTimezone(
            e['startTime'],
            venueTimezoneMap[Number(e['venueStateRefId'])] != null
              ? venueTimezoneMap[Number(e['venueStateRefId'])].timezone
              : null,
  ...
  async import(
    ...
    const requiredField = ['competitionId', 'Match ID'];
    ...
    for (let i of importArr) {
      ...
      if (isNotNullAndUndefined(competition)) {
        ...
        if (
          isNotNullAndUndefined(i['Umpire 1 Organisation']) &&
          ...
          if (i['Umpire 1 Organisation'].length > 0) {
            let linkedCompOrgArray = await this.linkedCompetitionOrganisationService.findByNameAndCompetitionId(
            ...
            if (isArrayPopulated(linkedCompOrgArray)) {
              ...
              var rosterStatus;
              if (
                isNotNullAndUndefined(i['Umpire 1 Response']) &&
                i['Umpire 1 Response'].length > 0
              ) {
                if (
                  i['Umpire 1 Response'] == 1 ||
            ...
            message[`Line ${i.line}`].message.push(
              'Could not find a matching Umpire 1 Organisation.',
            );
          }
        }

        if (
          isNotNullAndUndefined(i['Umpire 2 Organisation']) &&
          ...
          if (i['Umpire 2 Organisation'].length > 0) {
            ...
            if (isArrayPopulated(linkedCompOrgArray)) {
              ...
              var rosterStatus;
              if (
                isNotNullAndUndefined(i['Umpire 2 Response']) &&
                i['Umpire 2 Response'].length > 0
              ) {
                if (
                  i['Umpire 2 Response'] == 1 ||
            ...
            message[`Line ${i.line}`].message.push(
              'Could not find a matching Umpire 2 Organisation.',
            );
          }
        }
        ...
        if (!umpire1Imported && !umpire2Imported) {
          ...
          message[`Line ${i.line}`].message.push(
            'Missing umpires information/Some information provided is wrong.',
          );
        }
      } else {
  ...
  async umpirePayments(
    ...
    if (isNotNullAndUndefined(OFFSET) && isNotNullAndUndefined(LIMIT)) {
      return {
        page: paginationData(paymentsData.matchCount, LIMIT, OFFSET).page,
        players: paymentsData.result,
      };
    } else {
  ...
  async exportUmpirePayments(
    ...
    if (
      ...
      return response
        .status(200)
        .send({ name: 'search_error', message: 'Required fields are missing' });
    }
    ...
    if (isArrayPopulated(paymentsDup)) {
      paymentResult = paymentsDup.map(e => {
        ...
        e['Authoriser'] =
          (e.approvedByUser !== null ? e.approvedByUser.firstName : '') +
          ' ' +
          (e.approvedByUser !== null ? e.approvedByUser.lastName : '');
    ...
    response.setHeader('content-type', 'text/csv');
    fastcsv
      .write(paymentResult, { headers: true })
      .on('finish', function () {})
      .pipe(response);
  }
}
