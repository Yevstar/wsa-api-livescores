...
export class TeamController extends BaseController {
  ...
  async delete(@Param('id') id: number, @HeaderParam('authorization') user: User) {
    ...
    await Promise.all(
      deletdUREs.map((ure: UserRoleEntity) => {
  ...
  async listCompetitionTeams(
    ...
    if (offset === 0 && limit === 0) {
      ...
      return {
        page: paginationData(parseInt(teamData.teamCount), limit, offset),
  ...
  async loadTeamLadderDetails(
    ...
    if (isNotNullAndUndefined(competitionUniqueKey)) {
      const getCompetitions = await this.competitionService.getCompetitionByUniquekey(
  ...
  async loadTeamLadder(
    ...
    try {
      ...
      if (isNotNullAndUndefined(requestBody.competitionId)) {
        const getCompetition = await this.competitionService.getCompetitionByUniquekey(
          requestBody.competitionId,
        );
  ...
  async getPlayerList(
    ...
    logger.info(
      `TeamController - invite : teamIds ${teamIds}, playerIds ${playerIds} from user (id: ${user.id.toString()} firstName: ${
        user.firstName
    ...
    if (player.userId) {
      ...
      if (isInviteToParents && emailUser) {
        ...
        if (
          ...
          playerUser.firstName == player.firstName &&
          playerUser.lastName == player.lastName
        ) {
          if (!emailUser) {
            playerUser.email = player.email;
  ...
  private async notifyInvite(email: string) {
    ...
    if (invitedUser) {
      let tokens = (await this.deviceService.getUserDevices(invitedUser.id)).map(
  ...
  async addTeam(
    ...
    if (
      isNullOrEmpty(teamData.name) ||
      isNullOrEmpty(teamData.competitionId) ||
      isNullOrEmpty(teamData.divisionId)
    ) {
    ...
    if (!teamData.id) {
      const existing = await this.teamService.findByNameAndCompetition(
        teamData.name,
        teamData.competitionId,
      );
    ...
    if (teamData.email && teamData.firstName && teamData.lastName && teamData.mobileNumber) {
      ...
      if (foundUser) {
        if (
          foundUser.firstName == teamData.firstName &&
          foundUser.lastName == teamData.lastName &&
          foundUser.mobileNumber == teamData.mobileNumber
        ) {
          ...
          return response.status(400).send({
            name: 'validation_error',
            message:
              'A user with this email address already exists however other details do not match',
          });
        }
      } else {
    ...
    for (let managerId of managerIds) {
      if (managerId != 0) {
        if (!savedUser) {
          await this.userService.deleteRolesByUser(
            managerId,
            Role.MEMBER,
            savedTeam.competitionId,
    ...
    existingManagerUREs.forEach(async ure => {
      ...
      if (!found) {
        ...
        const chatPermissionCount = await this.ureService.getUserTeamChatRoleCount(
          savedTeam.id,
          managerUser.id,
        );
    ...
    if (file && isPhoto(file.mimetype)) {
      if (
        isNotNullAndUndefined(teamData.logoUrl) &&
        teamData.logoUrl.includes(`%2Fteam%2F${savedTeam.id}`)
      ) {
        const fileName = await this.firebaseService.getFileNameFromUrl(
          JSON.stringify(teamData.logoUrl),
          'team%F',
        );
      ...
      if (result) {
        ...
        return response
          .status(400)
          .send({ name: 'save_error', message: 'Logo not saved, try again later.' });
      }
    } else if (isNullOrEmpty(teamData.logoUrl) && savedTeam.competitionOrganisationId) {
      let linkedCompetitionOrganisation = await this.competitionOrganisationService.findLinkedCompetitionOrganisation(
        stringTONumber(savedTeam.competitionOrganisationId),
      );
      if (
        isNotNullAndUndefined(linkedCompetitionOrganisation) &&
  ...
  async importTeam(
    ...
    const requiredField = ['Team Name', 'Division Grade', 'Organisation'];
    ...
    if (isNotNullAndUndefined(competition)) {
      for (let i of importArr) {
        let teamData = await this.teamService.findByNameAndCompetition(
          i['Team Name'],
          competitionId,
          null,
          i['Division Grade'],
        ...
        if (!isArrayPopulated(teamData)) {
          let divisionData = await this.divisionService.findByName(
            i['Division Grade'],
            competitionId,
          );
          let linkedCompetitionOrganisationData = await this.linkedCompetitionOrganisationService.findByNameAndCompetitionId(
            i.Organisation,
            competitionId,
          );
          if (
            !isArrayPopulated(divisionData) ||
            !isArrayPopulated(linkedCompetitionOrganisationData)
          ) {
            ...
            if (!isArrayPopulated(divisionData)) {
              message[`Line ${i.line}`].message.push(
                `Can't find a matching Division Grade "${i['Division Grade']}" related to current competition: ${competition.longName}.`,
              );
            }
            if (!isArrayPopulated(linkedCompetitionOrganisationData)) {
              message[`Line ${i.line}`].message.push(
                `Can't find a matching organisation "${i.Organisation}" related to current competition: ${competition.longName}.`,
              );
            }
          } else {
            let team = new Team();
          ...
          message[`Line ${i.line}`].message.push(
            `The team "${i['Team Name']}" is already registered.`,
          );
        }
      }
  ...
  async exportTeams(
    ...
    fastcsv
      .write(getTeamsData, { headers: true })
      .on('finish', function () {})
      .pipe(response);
  }
  ...
  async saveTeamLadderAdjustments(
    ...
    try {
      ...
      const existingTeamLadderAdjustments = await this.teamLadderService.findExistingTeamLadderAdj(
        competitionId,
  ...
  async getTeamLadderAdjustments(
    ...
    try {
      ...
      const ladderAdjustments = await this.teamLadderService.getTeamLadderAdjustments(
        competitionId,
        divisionId,
      );
