...
export class UmpireController extends BaseController {
  ...
  async index(
    ...
    crudResponse.data = await this.umpireService.addOrganisationNameToUmpiresWithURE(
      crudResponse.data,
    );
  ...
  async getDetailedUmpire(
    ...
    const { organisationId, umpireId } = await this.validate(
      response,
      queryParams,
      OrganisationQueryParam,
    );
  ...
  async updateRank(
    ...
    const { competitionId, umpireId } = await this.validate(
      response,
      params,
      RankUmpireRouteParams,
    );
