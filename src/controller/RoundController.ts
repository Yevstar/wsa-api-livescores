...
export class RoundController extends BaseController {
  ...
  @Authorized()
  @Get('/list')
  async findByMatchIds(@QueryParam('ids') roundIds: number[] = []): Promise<Round[]> {
  ...
  @Authorized()
  @Post('/')
  async create(@Body() round: Round): Promise<Round> {
  ...
  @Authorized()
  @Get('/name')
  async findUniqueNames(@QueryParam('competitionId') competitionId: number): Promise<Round[]> {
