...
export class MnbController extends BaseController {
    ...
    async scores() {
        ...
        for (let i = 0; i < matches.length; i++) {
            ...
            await submitScore(match.mnbMatchId, match.team1Score, match.team2Score, mnbUrl, mnbUser, mnbPassword);
    ...
    async attendance() {
        ...
        const groupedAttendances = _.groupBy(attendances, (attendance: Attendance) => attendance.match.mnbMatchId);
        for (let mnbMatchId in groupedAttendances) {
            ...
            await submitAttendance(mnbMatchId, team1PlayerIds, team2PlayerIds, mnbUrl, mnbUser, mnbPassword);

            // Update the submission flags on the attendance records.
            await Promise.all(groupedAttendances[mnbMatchId].map((attendance: Attendance) => {
