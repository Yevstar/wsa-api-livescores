...
export class MatchController extends BaseController {
  ...
  @Authorized()
  @Delete('/id/:id')
  async delete(@Param('id') id: number, @HeaderParam('authorization') user: User) {
  ...
  @Authorized()
  @Get('/admin/:id')
  async getAdmin(@Param('id') id: number, @QueryParam('lineups') lineups: number = 0) {
    ...
    if (matchDetails && matchDetails.match[0]) {
      const competition = await this.competitionService.findById(
        matchDetails.match[0].competitionId,
      );
      const linkedCompetitionOrganisation = await this.linkedCompetitionOrganisationService.findByOrganisationId(
  ...
  @Get('/list')
  async findByMatchIds(@QueryParam('ids') matchIds: number[] = []): Promise<Match[]> {
  ...
  async find(
    ...
    if (competitionOrganisationId) {
      const competitionOrganisation = await this.competitionOrganisationService.findById(
    ...
    if (
      isNotNullAndUndefined(matchFound.matchCount) &&
      isNotNullAndUndefined(matchFound.result) &&
      limit
    ) {
  ...
  async loadHomeMatches(
    ...
    if (competitionOrganisationIds && !Array.isArray(competitionOrganisationIds))
    ...
    if (
      ...
      let upcoming = await this.matchService.loadHomeUpcoming(
      ...
      if (upcomingCount != undefined && upcomingCount != 0) {
        ...
        if (isArrayPopulated(competitionOrganisationIds)) {
          teams = _.uniq([
            ...teamIds,
            ...(
              await this.teamService.teamIdsByCompetitionOrganisationIds(competitionOrganisationIds)
            ).map(data => data['id']),
          ]);
        }
        let filtered: Match[] = [];
        for (const mh of upcoming) {
          if (!filtered.some(el => el.id === mh.id)) {
            ...
            let t1 = contain(teams, team1)
              ? filtered.filter(m => m.team1Id == team1).length
              : upcomingCount;
            let t2 = contain(teams, team2)
              ? filtered.filter(m => m.team2Id == team2).length
              : upcomingCount;
      ...
      const ended = endTimeRange
        ? await this.matchService.loadHomeEnded(competitionOrganisationIds, teamIds, endTimeRange)
        : [];
  ...
  async admin(
    ...
    if (competitionId && requestFilter) {
      ...
      return this.matchService.loadAdmin(competitionId, teamId, roleId, userId, requestFilter, {
  ...
  async create(
    ...
    try {
      ...
      if (match.id == 0) {
        ...
        if (
          (dbMatch.divisionId != match.divisionId ||
        ...
        if (newUmpires) {
          if (oldUmpires) {
            ...
            for (let oldUmpire of oldUmpires) {
              ...
              if (
                newUmpires.filter(
                  umpire => umpire.umpireName && umpire.sequence == oldUmpire.sequence,
                ).length == 0
              ) {
                matchUmpireDeletePromises.push(this.matchUmpireService.deleteById(oldUmpire.id));
              }
            }
          ...
          for (let newUmpire of newUmpires) {
            if (newUmpire.umpireName) {
              ...
              if (oldUmpires) {
                oldMatchingUmpireList = oldUmpires.filter(
                  umpire => umpire.sequence == newUmpire.sequence,
                );
              }
              if (oldMatchingUmpireList.length > 0) {
                let oldUmpire = oldMatchingUmpireList[0];
                if (
                  oldUmpire.competitionOrganisationId != newUmpire.competitionOrganisationId ||
      ...
      let oldRosters = (await this.rosterService.findAllRostersByParam([match.id])).filter(
        roster =>
      ...
      if (isArrayPopulated(match.rosters)) {
        ...
        for (let oldRoster of oldRosters) {
          ...
          if (matchedNewRosters.length == 0) {
            deleteRosterPromises.push(this.deleteRoster(oldRoster));
          }
        }
        ...
        for (let newRoster of match.rosters) {
          const isAvailableToAssign = await this.matchService.checkIfAbleToAssignUmpireToMath(
            match.id,
            newRoster.userId,
          );
        ...
        for (let roster of oldRosters) {
          deleteRosterPromises.push(this.deleteRoster(roster));
        }
        await Promise.all(deleteRosterPromises);
      }
      ...
      this.sendMatchEvent(saved, false, {
        nonSilentNotify: nonSilentNotify,
        mandatorySilentNotifyUserIds: mandatorySilentNotifyUserIds,
  ...
  private async addUmpireTypeRoster(
    ...
    let matchedOldRosters = oldRosters.filter(
      roster => roster.roleId == roleId && roster.userId && roster.userId == newRoster.userId,
    );
    ...
    if (matchedOldRosters.length == 0 && newRoster.userId) {
      ...
      if (roleId == Role.UMPIRE && isNotNullAndUndefined(oldUmpires) && oldUmpires.length > 0) {
        ...
        if (isNotNullAndUndefined(oldMU) && oldMU.sequence != umpireSequence) {
          ...
          let oldRoster = oldRosters.filter(
            r => r.roleId == Role.UMPIRE && r.userId == newRoster.userId,
          )[0];
  ...
  private async createUmpire(
    umpire: MatchUmpire,
    createdBy: number,
    existingUmpire: MatchUmpire = null,
  ) {
  ...
  private async deleteRoster(roster: Roster) {
    ...
    try {
      if (roster.roleId == Role.SCORER) {
        ...
        if (roster.userId) {
          await this.umpireRemoveRoster(Role.UMPIRE_RESERVE, roster.userId, roster.matchId);
        }
      } else if (roster.roleId == Role.UMPIRE_COACH) {
        if (roster.userId) {
          await this.umpireRemoveRoster(Role.UMPIRE_COACH, roster.userId, roster.matchId);
        }
      } else {
        if (roster.userId) {
          ...
          await this.umpireRemoveRoster(Role.UMPIRE, roster.userId, roster.matchId);
        }
      }
    } catch (error) {
  ...
  private async removeScorerRoster(matchId: number, roster: Roster) {
    try {
      ...
      let tokens = (await this.deviceService.findScorerDeviceFromRoster(matchId, roster.id)).map(
  ...
  async addMatchScores(
    ...
    if (scores) {
      ...
      if (
        match &&
        (match.team1Score != scores.team1Score || match.team2Score != scores.team2Score)
      ) {
  ...
  async updateScore(
    ...
    if (gameStatCode) {
      this.matchEventService.logMatchEvent(
        ...
        user.id,
        'team' + teamSequence,
        recordPoints ? points.toString() : positionId ? positionId.toString() : '',
        'playerId',
    ...
    if (recordAssistPlayer && isNotNullAndUndefined(assistPlayerId)) {
      this.matchEventService.logMatchEvent(
        ...
        user.id,
        'team' + teamSequence,
        assistPlayerPositionId ? assistPlayerPositionId.toString() : '',
        'playerId',
        assistPlayerId ? assistPlayerId.toString() : '',
      );
    }
    return match;
  }
  ...
  async updateStats(
    ...
    if (
      ...
      (recordAdditionalTime && !isNotNullAndUndefined(additionalMs))
    ) {
      return response.status(400).send({
        name: 'validation_error',
    ...
    if (recordPoints) {
      ...
      if (gameStatCode == GameStatCodeEnum.F) {
        ...
        let newMatchStartTime = new Date(
          match.startTime.getTime() + (msFromStart - msChangeToStartTime),
        );
  ...
  private getRecordPointsAttribute1Value(gameStatCode: string, points: number, foul: string) {
    switch (gameStatCode) {
  ...
  async startMatch(
    ...
    this.sendMatchEvent(match, false, { user: user });
    await this.matchEventService.logLiteMatchEvent(
      matchId,
      'timer',
      'start',
      1,
  ...
  async resumeMatch(
    ...
    if (match) {
      ...
      if (!totalPausedTime) {
        ...
        let millisecond = match.pauseStartTime
          ? match.pauseStartTime.getTime() - match.startTime.getTime()
          : 0;
  ...
  async endMatch(
    ...
    if (match) {
      ...
      if (!isNotNullAndUndefined(dbMatch)) {
        if (!isNotNullAndUndefined(dbMatch.matchStatus) || dbMatch.matchStatus != 'ENDED') {
        ...
        } else if (isNotNullAndUndefined(dbMatch.matchStatus) && dbMatch.matchStatus == 'ENDED') {
          if (match.matchAction == 'VERIFY_SCORES') {
            match.matchAction = 'VERIFY_ACTION_LOGS';
          } else if (match.matchAction == 'VERIFY_ACTION_LOGS') {
            match.matchAction = sendOffReportRequired ? 'SEND_OFF_REPORT' : null;
          }
        }
      }
  ...
  async sendMatchEvent(
    ...
    optionals?: {
      user?: User;
      subtype?: string;
      nonSilentNotify?: boolean;
      mandatorySilentNotifyUserIds?: number[];
    },
  ) {
    try {
      if (match) {
        ...
        if (
          ...
          if (
            ...
            isNotNullAndUndefined(dbMatch.competition) &&
        ...
        let list = await this.watchlistService.loadByParam(match.id, [
          match.team1Id,
          match.team2Id,
        ]);
        ...
        if (
          !isNullOrEmpty(messageBody) &&
          isNotNullAndUndefined(optionals) &&
          isArrayPopulated(optionals.mandatorySilentNotifyUserIds)
        ) {
          let silentNotifyDevices = await this.deviceService.getUserTokens(
            optionals.mandatorySilentNotifyUserIds,
          );
  ...
  async loadLineups(
    ...
    return this.matchService.findLineupsByParam(
      matchId,
      competitionId,
      teamId,
      playerId,
  ...
  @Authorized()
  @Patch('/lineup/status')
  async updateLineupService(@Body() lineup: Lineup, @Res() response: Response) {
    ...
    if (match.matchStatus != 'ENDED') {
      if (lineups.length > 0) {
        ...
        if (updateGameTimeAttendances) {
          this.updateGameTimeAttendancesViaLineup(matchId, teamId, lineups, currentUser);
        }
        let tokens = (await this.deviceService.findScorerDeviceFromRoster(matchId)).map(
          device => device.deviceId,
        );
        if (tokens && tokens.length > 0) {
          this.firebaseService.sendMessageChunked({
        ...
        return response
          .status(400)
          .send({ name: 'updated_lineup_error', message: 'List lineup is empty' });
      }
    } else {
      return response.status(400).send({
  ...
  private async checkLineupsForExisting(matchId: number, teamId: number, lineups: Lineup[]) {
    let existingLineups = await this.lineupService.findByParams(matchId, null, teamId, null);
    lineups.forEach(lineup => {
      if (!lineup.id) {
        const existingLineup = existingLineups.filter(
          el =>
  ...
  private async updateGameTimeAttendancesViaLineup(
    ...
    if (isArrayPopulated(lineups)) {
      ...
      await this.gameTimeAttendanceService.deleteByMatchAndTeam(matchId, teamId, null, true, null);
      var createGTAs = [];
      lineups.forEach(lineup => {
  ...
  async deleteLineup(
    ...
    if (lineup) {
      ...
      await await this.gameTimeAttendanceService.deleteByMatchAndTeam(
        lineup.matchId,
        lineup.teamId,
        0,
        true,
      ...
      let tokens = (await this.deviceService.findScorerDeviceFromRoster(lineup.matchId)).map(
        device => device.deviceId,
      );
      if (tokens && tokens.length > 0) {
        this.firebaseService.sendMessageChunked({
  ...
  private async updateMatchEventsForLineup(match: Match, teamId: number, lineups: Lineup[]) {
    let gsPlayerId;
    let gaPlayerId;
      ...
      for (let item of matches) {
        if (item.matchStatus == 'ENDED') {
          ...
          if (ladderSettings == undefined) {
            ladderSettings = await this.competitionLadderSettingsService.getByCompetitionDivisionId(
              item.competitionId,
              item.divisionId,
            );
          ...
          if (isArrayPopulated(ladderSettings)) {
            let teamLadderList = await this.teamLadderService.getTeamLadderByMatch(
              item,
              ladderSettings,
              userId,
            );
  ...
  async bulkMatchPushBack(
    ...
    let matchesData = await this.matchService.findByDate(
      new Date(startTimeStart),
      new Date(startTimeEnd),
      competitionId,
    );
    ...
    if (arr.length > 0) {
      ...
      if (nonSilentNotifyMatches.length > 0) {
        this.sendBulkMatchUpdateNotification(nonSilentNotifyMatches, {
          subtype: 'bulk_time_matches',
          nonSilentNotify: true,
        });
      }
      if (silentNotifyMatches.length > 0) {
        this.sendBulkMatchUpdateNotification(silentNotifyMatches, { subtype: 'bulk_time_matches' });
      }
    }
  ...
  async doubleHeader(
    ...
    if (isArrayPopulated(firstRoundArray) && isArrayPopulated(secondRoundArray)) {
            ...
            if (isArrayPopulated(firstMatchesArray) && isArrayPopulated(secondMatchesArray)) {
              ...
              for (let m2 of secondMatchesArray) {
                if (m2.type == 'FOUR_QUARTERS') {
                  m2.startTime = new Date(
                    m2.startTime.getTime() +
                      (m2.matchDuration / 2 + m2.breakDuration + m2.mainBreakDuration),
                  );
                  m2.type = 'TWO_HALVES';
      ...
      if (arr.length > 0) {
        ...
        if (silentNotifyMatches.length > 0) {
          this.sendBulkMatchUpdateNotification(silentNotifyMatches, {
            subtype: 'bulk_double_header',
          });
        }
  ...
  async importMatch(
    ...
    for (let i of importArr) {
      ...
      let team1Data = await this.teamService.findByNameAndCompetition(
        i['Home Team'],
        competitionId,
        null,
        divisionData.length > 0 ? divisionData[0].name : undefined,
      );
      let team2Data = await this.teamService.findByNameAndCompetition(
        i['Away Team'],
        competitionId,
        null,
        divisionData.length > 0 ? divisionData[0].name : undefined,
      );
      let venueData = await this.competitionVenueService.findByCourtAndVenueName(
        i['Venue'],
        i['Court'],
        competitionId,
      );
      ...
      if (!!divisionData[0] && !!team1Data[0] && !!team2Data[0] && !!venueData[0]) {
        let rounds = await this.roundService.findByName(
          competitionId,
          i['Round'],
          divisionData[0].id,
        );
        ...
        if (!divisionData[0])
          message[`Line ${i.line}`].message.push(
            'The division entered is not associated with this competition.',
          );
        if (!team1Data[0])
          message[`Line ${i.line}`].message.push(`Can't find the team named "${i['Home Team']}"`);
        if (!team2Data[0])
          message[`Line ${i.line}`].message.push(`Can't find the team named "${i['Away Team']}"`);
        if (!venueData[0])
          message[`Line ${i.line}`].message.push(
            `Can't find the court named "${i['Court']}" on the venue "${i['Venue']}"`,
          );
      }
    }
  ...
  async bulkUpdateCourts(
    ...
    const getMatches = await this.matchService.getMatchDetailsForVenueCourtUpdate(
      competitionId,
      startTime,
      endTime,
      fromCourtIds,
    ...
    if (isArrayPopulated(getMatches)) {
      ...
      return response
        .status(200)
        .send({ success: true, message: 'venue courts update successfully' });
    } else {
      return response.status(212).send({
        success: false,
  ...
  async exportMatches(
    ...
    if (isArrayPopulated(getMatchesData)) {
      ...
      getMatchesData.map(e => {
        ...
        e['Start Time'] = convertMatchStartTimeByTimezone(
          e.startTime,
          locationId != null ? locationsTimezoneMap[locationId].timezone : null,
    ...
    fastcsv
      .write(getMatchesData, { headers: true })
      .on('finish', function () {})
      .pipe(response);
  }
  ...
  private async sendBulkMatchUpdateNotification(
    ...
    try {
      ...
      if (
        ...
        messageBody =
          'A number of changes have been made to your matches. Please check match details.';
      }
  ...
  async print(
    ...
    try {
      const competition = await this.competitionService.findById(competitionId);
      const organisation = await this.linkedCompetitionOrganisationService.getOrganisationLogoDetails(
        competition.organisationId,
      );

      this.matchService
        .printMatchSheetTemplate(
          templateType,
          user,
          organisation,
          competition,
  ...
  @Authorized()
  @Post(`/livestreamURL`)
  async updateLivestreamURL(@Body() match: Match, @Res() response: Response): Promise<any> {
    if (match && isNotNullAndUndefined(match.id) && isNotNullAndUndefined(match.livestreamURL)) {
  ...
  async deleteMatchEvent(
    ...
    if (
      this.matchEventService.isGameStatGoalOrPoints(gameStatCode) &&
      ...
      for (let me of matchEvents) {
        if (
          ...
          (me.eventCategory == 'score' &&
            !isNotNullAndUndefined(team1Score) &&
            !isNotNullAndUndefined(team2Score))
        ) {
