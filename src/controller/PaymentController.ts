...
export class PaymentController extends BaseController {
  ...
  async createStripeTransfersToUmpireAccount(
    ...
    try {
      ...
      const currentOrgDetails = await this.organisationService.findOrganisationByUniqueKey(
        organisationKey,
        authToken,
      );
        ...
        if (isArrayPopulated(transfersBody.transfers)) {
          ...
          for (let i of transfersBody.transfers) {
            if (STATUS === 1) {
              ...
              const STRIPE_AMOUNT = await this.matchUmpireService.calculatePaymentForUmpire(
                i.matchUmpireId,
              ...
              try {
                await this.stripeService.makePaymentIntent(
                  STRIPE_AMOUNT,
                  stripeId,
                  orgCustomerId,
                  orgPmId,
      ...
      return response
        .status(400)
        .send(`Error in sending transfer to another stripeAccount: ${err}`);
    }
  }
}
