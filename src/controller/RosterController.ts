...
export class RosterController extends BaseController {
  ...
  async delete(
    ...
    if (roster) {
      ...
      if (result) {
        switch (category) {
          case 'Scoring':
            let scorerDeviceTokens = (
              await this.deviceService.findScorerDeviceFromRoster(undefined, id)
          ...
          case 'UmpireReserve':
          case 'UmpireCoach':
            let umpireDeviceTokens = (await this.deviceService.getUserDevices(roster.userId)).map(
            ...
            break;
          default:
            let tokens = (await this.deviceService.getUserDevices(roster.userId)).map(
              device => device.deviceId,
            );
  ...
  @Authorized()
  @Get('/match')
  async findByMatchIds(@QueryParam('ids') matchIds: number[] = [], @Res() response: Response) {
  ...
  async updateStatus(
    ...
    if (roster) {
      ...
      switch (category) {
        case 'Scoring':
          let scoringDeviceTokens = (await this.deviceService.findManagerDevice(result.teamId)).map(
            device => device.deviceId,
          );
          ...
          break;
        case 'Playing':
          let managerAndCoachDeviceTokens = (
            await this.deviceService.findManagerAndCoachDevices(roster.teamId)
          ).map(device => device.deviceId);
          ...
          break;
        case 'Event':
          let eventDeviceTokens = (
            await this.deviceService.getUserDevices(roster.eventOccurrence.created_by)
          ).map(device => device.deviceId);
        ...
        case 'UmpireReserve':
        case 'UmpireCoach':
          let umpireDeviceTokens = (await this.deviceService.findManagerDevice(result.teamId)).map(
            device => device.deviceId,
          );
          if (umpireDeviceTokens && umpireDeviceTokens.length > 0) {
            if (status == 'NO') {
  ...
  @Authorized()
  @Post('/admin/assign')
  async addAdminRoster(@Body() roster: Roster, @Res() response: Response) {
    if (!roster) {
      return response.status(400).send({
        name: 'validation_error',
        message: 'Roster in body required.',
    ...
    var existingRoster = await this.rosterService.getRosterStatus(
      roster.roleId,
      roster.teamId,
      roster.matchId,
    );
    ...
    if (existingRoster) {
      let tokens = (await this.deviceService.getUserDevices(existingRoster.userId)).map(
        device => device.deviceId,
      );
      if (tokens && tokens.length > 0) {
        this.firebaseService.sendMessageChunked({
  ...
  async deleteAdminRoster(
    ...
    return await this.delete(id, category, response);
  }
  ...
  async exportScorer(
    ...
    if (
      ...
      fastcsv
        .write(getScorersData.users, { headers: true })
        .on('finish', function () {})
        .pipe(response);
    } else {
  ...
  async exportUmpire(
    ...
    if (
        ...
        dict.results.map(e => {
          ...
          if (isArrayPopulated(e['user']['userRoleEntities'])) {
            for (let i of e['user']['userRoleEntities']) {
              orgArray.push(i['linkedCompetitionOrganisation']['name']);
            }
          }
      ...
      response.setHeader('content-type', 'text/csv');
      fastcsv
        .write(dict.results, { headers: true })
        .on('finish', function () {})
        .pipe(response);
    } else {
      return response.status(212).send({
  ...
  private async evaluateWatchlist(roster: Roster, createdBy: number) {
    let deviceIds = (await this.deviceService.getUserDevices(roster.userId)).map(
      device => device.deviceId,
    );
    if (isArrayPopulated(deviceIds)) {
      ...
      for (let watchItem of userWatchlist) {
        if (watchItem.entityTypeId == EntityType.TEAM && roster.teamId == watchItem.entityId) {
      ...
      if (needToSetWatch) {
        await this.createWatchlist(roster.userId, roster.teamId, createdBy, deviceIds);
      }
    }
  }
