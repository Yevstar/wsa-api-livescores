...
export class BaseController {
  ...
  protected async notifyRosterChange(
    ...
    switch (category) {
      case 'Scoring':
        let tokens = [];
        let managerDeviceTokens = (await this.deviceService.findManagerDevice(roster.teamId)).map(
        ...
        let scorerDeviceTokens = (
          await this.deviceService.findScorerDeviceFromRoster(roster.matchId, roster.id)
