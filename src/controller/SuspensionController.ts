...
export class SuspensionController {
  ...
  @Authorized()
  @Post('/')
  async createSuspension(@Body() suspensionDto: SuspensionDto, @Res() response: Response) {
    ...
    const createdSuspension = await this.suspensionService.createOne(suspensionDto);
  ...
  async updateSuspension(
    ...
    const updatedSuspension = await this.suspensionService.updateOne(suspensionId, suspensionDto);
