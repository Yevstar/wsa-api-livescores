...
async function submitScore(
  ...
  try {
    ...
    // Submit the score.
    await page.goto(
      `${mnbUrl}/pages/admin/matchscore_12.aspx?&matchID=${mynetballMatchId}&popup=1`,
    );
    ...
    if ((await page.$(`#ctl00_MainPlaceHolder_dlScores_ctl00_dg1_ctl06_tbScore_1`)) !== null) {
      await type(
        page,
        '#ctl00_MainPlaceHolder_dlScores_ctl00_dg1_ctl06_tbScore_1',
        `${team1Score}`,
      );
      await type(
        page,
        '#ctl00_MainPlaceHolder_dlScores_ctl01_dg1_ctl06_tbScore_1',
        `${team2Score}`,
      );
    }
    ...
    if ((await page.$(`#ctl00_MainPlaceHolder_dlScores_ctl00_dg1_ctl04_tbScore_1`)) !== null) {
      await type(
        page,
        '#ctl00_MainPlaceHolder_dlScores_ctl00_dg1_ctl04_tbScore_1',
        `${team1Score}`,
      );
      await type(
        page,
        '#ctl00_MainPlaceHolder_dlScores_ctl01_dg1_ctl04_tbScore_1',
        `${team2Score}`,
      );
    }

    await page.select(
      '#ctl00_MainPlaceHolder_dlScores_ctl00_dlResultType',
      `${team1Score > team2Score ? WON : team1Score === team2Score ? DRAWN : LOST}`,
    );
    await page.select(
      '#ctl00_MainPlaceHolder_dlScores_ctl01_dlResultType',
      `${team2Score > team1Score ? WON : team2Score === team1Score ? DRAWN : LOST}`,
    );
    const wait = page.waitForNavigation({ timeout: TIMEOUT });
...
async function submitAttendance(
  ...
  try {
    ...
    const otherTeamOption = await page.evaluate(() => {
      return document.querySelector('#ctl00_MainPlaceHolder_dlTeam>option:not([selected])')[
        'value'
      ];
    });
    const wait = page.waitForNavigation({ timeout: TIMEOUT });
...
async function setTeamIds(page, teamIds) {
  ...
  for (let i = 0; i < mnbPlayerIds.length; i++) {
    ...
    await page.evaluate(mnbPlayerId => {
      ...
      return document
        .querySelector('#addplayer_dialog')
...
export { submitScore, submitAttendance };
