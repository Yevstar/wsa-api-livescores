    ...
    filedList.forEach(field => {
      if (isNullOrEmpty(val[field])) {
        ...
        if (field === 'dateOfBirth' || field === 'DOB' || field === 'Date') {
          const date =
            typeof tempValue[field] === 'string'
              ? parseDateString(tempValue[field])
              : tempValue[field];
...
export function parseDateTimeZoneString(date, time, timezone) {
  ...
  return `${zeroFill(4, dateArray[2])}-${zeroFill(2, dateArray[1])}-${zeroFill(
    2,
    dateArray[0],
  )}T${zeroFill(2, hr)}:${zeroFill(2, min)}:00.000+${zeroFill(2, timeZoneString)}:00`;
}
