...
export function getMatchUmpireNotificationMessage(
  ...
  if (rosterLocked) {
    messageBody =
      `${match.competition.name} has sent you a ` + `new Umpiring Duty for ${matchTime}.`;
  } else {
    messageBody =
      `${match.competition.name} has sent you a ` +
...
export function getMatchUpdatedNonSilentNotificationMessage(
  ...
  if (isNotNullAndUndefined(venueDetails) && venueDetails.length > 0) {
    messageBody =
      `A change has been made to your match on ` +
      `${matchTime} on ${venueDetails}. Please check match details.`;
  } else {
    messageBody =
      `A change has been made to your match on ` + `${matchTime}. Please check match details.`;
  }
