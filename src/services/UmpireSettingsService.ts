  ...
  async saveAllocationSettings(
    ...
    if ((settings.umpireAllocationSettings || []).length) {
      for (const settingData of settings.umpireAllocationSettings) {
        ...
        response.umpireAllocationSettings.push(await this.entityManager.save(setting));
      }
    }
    if (!!settings.noUmpiresSetting) {
      const setting = await this.entityManager.create(
        NoUmpiresUmpireAllocationSetting,
        settings.noUmpiresSetting,
      );
      ...
      if (
        !settings.noUmpiresSetting.allDivisions &&
        (settings.noUmpiresSetting.divisions || []).length
      ) {
