...
export default class TeamLadderService extends BaseService<TeamLadder> {
  ...
  public async getTeamLadderByMatch(
    match: Match,
    competitionLadderSettings: any,
    userId,
  ): Promise<TeamLadder[]> {
    let teamLadderArr = [];
    try {
  ...
  private async deleteExistingData(match: Match, userId) {
    try {
      await this.entityManager
        .createQueryBuilder(TeamLadder, 'teamLadder')
        .update(TeamLadder)
  ...
  private async deleteExistingPoints(match: Match, userId) {
    try {
      await this.entityManager
        ...
        .andWhere('teamLadder.matchId = :matchId and teamLadder.teamLadderTypeRefId <= 9', {
          matchId: match.id,
        })
        .execute();
      ...
      await this.entityManager
        ...
        .update(TeamLadder)
        .set({ deleted_at: new Date(), updatedBy: userId, updated_at: new Date() })
        .andWhere('teamLadder.matchId = :matchId and teamLadder.teamLadderTypeRefId >= 21 ', {
          matchId: match.id,
        })
        .execute();
    } catch (error) {
    ...
    try {
      if (divisionId != null) {
        await this.entityManager
          ...
          .set({ deleted_at: new Date(), updatedBy: userId, updated_at: new Date() })
          .andWhere(
            'teamLadder.competitionId = :competitionId and  teamLadder.divisionId = :divisionId',
        ...
        await this.entityManager
          ...
          .update(TeamLadder)
          .set({ deleted_at: new Date(), updatedBy: userId, updated_at: new Date() })
          .andWhere('teamLadder.competitionId = :competitionId', { competitionId: competitionId })
          .execute();
      }
    } catch (error) {
      throw error;
  ...
  public async findExistingTeamLadderAdj(
    competitionId: number,
    divisionId: number,
  ): Promise<TeamLadder[]> {
    try {
      let result = await this.entityManager
        .createQueryBuilder(TeamLadder, 'tl')
        .where(
          'tl.competitionId = :competitionId and tl.divisionId = :divisionId and tl.teamLadderTypeRefId = 25 and tl.deleted_at is null',
          { competitionId: competitionId, divisionId: divisionId },
        )
        .getMany();
  ...
  public async getTeamLadderAdjustments(
    competitionId: number,
    divisionId: number,
  ): Promise<TeamLadder[]> {
    try {
      let result = await this.entityManager.query(
        `Select tl.id as teamLadderId, tl.teamId, tl.teamLadderTypeValue as points, tl.adjustmentReason
                 ...
                 and tl.teamLadderTypeRefId = 25 and tl.deleted_at is null `,
