...
export default class MatchService extends BaseService<Match> {
  ...
  public async findByMnb(): Promise<Match[]> {
    return this.entityManager
      ...
      .select([
        ...
        'competition',
        'competition.mnbUser',
        'competition.mnbPassword',
        'competition.mnbUrl',
      ])
      .andWhere(
        "match.mnbMatchId IS NOT NULL AND match.mnbMatchId != '111' " +
  ...
  public async findMatchById(
    ...
    if (gameType == 'BASKETBALL') {
      ...
      if (includeSinBins) {
        ...
        query.leftJoinAndSelect('matchSinBins.matchEvent', 'matchEvent');
        query.leftJoinAndSelect('matchSinBins.player', 'player', 'player.deleted_at is null');
      }
    }
  ...
  public async findByParam(
    ...
    if (divisionIds != undefined && divisionIds != null)
      query.andWhere('match.divisionId in (:divisionIds)', { divisionIds });
    if (isNotNullAndUndefined(roundName) && roundName !== '')
      query.andWhere('round.name = :roundName', { roundName });
    if (isNotNullAndUndefined(search) && search !== '') {
      const search_ = `%${search.toLowerCase()}%`;
      query.andWhere(`(lower(team1.name) like :search1 or lower(team2.name) like :search2)`, {
        search1: search_,
        search2: search_,
      });
    }
  ...
  public async getMatchResultTypes(): Promise<MatchResultType[]> {
    return this.entityManager.createQueryBuilder(MatchResultType, 'mr').getMany();
  }

  public async loadHomeLive(
  ...
  public async loadHomeUpcoming(
    competitionOrganisationIds: number[],
    teamIds: number[],
    upcomingStartTimeRange: number,
  ): Promise<Match[]> {
    let query = this.entityManager.createQueryBuilder(Match, 'match');
    this.filterByOrganisationTeam(undefined, competitionOrganisationIds, teamIds, query);
    query.andWhere(
      ...
      query.andWhere(
        new Brackets(qb => {
          ...
          if (isArrayPopulated(competitionOrganisationIds)) {
            qb.orWhere(
              '(team1.competitionOrganisationId in (:competitionOrganisationIds) or team2.competitionOrganisationId in (:competitionOrganisationIds))',
  ...
  public async loadCompetitionAndDate(
    competitionId: number,
    start: Date,
    end: Date,
    live: boolean,
  ...
  public async loadAdmin(
    ...
    if (result != null) {
      ...
      if (options.showRosterAvailability) {
        responseObject['matches'] = await this.markUnavailableMatchesForUmpire(
  ...
  public async findLineupsByParam(
    matchId: number,
    competitionId: number,
    teamId: number,
    playerId: number,
  ...
  public async deleteLineups(matchId: number, teamId: number) {
    return this.entityManager
      ...
      .delete()
      .from(Lineup)
      .andWhere('matchId = :matchId and teamId = :teamId', { matchId, teamId })
      .execute();
  }
  ...
  public async findByDto({ from, to, competitionId, courtId, roundId }): Promise<Match[]> {
    let query = this.entityManager.createQueryBuilder(Match, 'match');
    if (from) query.andWhere('match.startTime >= :from', { from });
    if (to) query.andWhere('match.startTime <= :to', { to });
    if (competitionId) query.andWhere('match.competitionId = :competitionId', { competitionId });
  ...
  public async findTodaysMatchByParam(
    ...
    if (isNotNullAndUndefined(from) && isNotNullAndUndefined(to)) {
      query.andWhere(
        `((match.startTime < cast(:from as datetime) and match.matchStatus != 'ENDED') or
  ...
  public async getMatchDetailsForVenueCourtUpdate(
    competitionId: number,
    startTime: Date,
    endTime: Date,
    fromCourtIds: number[],
  ...
  public async getMatchTimezone(locationId: number): Promise<StateTimezone> {
    let query = await this.entityManager
      .createQueryBuilder(StateTimezone, 'stateTimezone')
    ...
    try {
      ...
      if (teamIds !== null) {
        filteredMatches = filteredMatches.filter(
          match => match.team1Id === teamIds || match.team2Id === teamIds,
        );
      }
      if (roundName !== null) {
      ...
      const createPDF = (html, options): Promise<Buffer> =>
      ...
      for (let i = 0; i < filteredMatches.length; i++) {
        ...
        if (
          isNotNullAndUndefined(currentMatch) &&
      ...
      if (filteredMatches.length > 0) {
        ...
        if (teamIds !== null) {
          teamName =
            filteredMatches[0].team1Id === teamIds
        ...
        const fileName = `${replaceStr(competition.name)}_${
          divisionIds === null ? 'All_Divisions_' : replaceStr(filteredMatches[0].division.name)
  ...
  public async deleteMatchFouls(matchId: number) {
    ...
    return this.entityManager
      .createQueryBuilder(MatchFouls, 'mouthFouls')
      .update()
  ...
  public async logMatchFouls(userId: number, matchId: number, teamSequence: number, foul: string) {
    ...
    switch (foul) {
      case 'P':
        teamSequence == 1 ? (matchFoul.team1Personal += 1) : (matchFoul.team2Personal += 1);
        break;
      case 'T':
        teamSequence == 1 ? (matchFoul.team1Technical += 1) : (matchFoul.team2Technical += 1);
        break;
      case 'D':
        teamSequence == 1
  ...
  public async deleteMatchTimeouts(matchId: number) {
    ...
    return this.entityManager
      .createQueryBuilder(MatchTimeout, 'matchTimeout')
      .update()
      .set({ deleted_at: endTime })
      .where('matchId = :matchId', { matchId })
      .execute();
  ...
  public async deleteMatchSinBin(matchId: number) {
    ...
    return this.entityManager
      .createQueryBuilder(MatchSinBin, 'matchSinBin')
      .update()
      .set({ deleted_at: endTime })
      .where('matchId = :matchId', { matchId })
      .execute();
  ...
  public async removeMatchSinBin(matchId: number, teamId: number, matchEventIds: number[]) {
    return this.entityManager
      .createQueryBuilder()
      .delete()
      .from(MatchSinBin)
  ...
  public async markUnavailableMatchesForUmpire(
    matches: Match[],
    umpireId: number,
    ...
    const rosters = await this.rosterService.getUmpireRostersForDates(
      umpireId,
      minStartTime,
      maxEndTime,
    );
    ...
    rosters.forEach(roster => {
      ...
      const endTime = roster.match.startTime.setMinutes(
        roster.match.startTime.getMinutes() + roster.match.matchDuration,
      );
    ...
    return matches.map(match => {
      ...
      const endTime = match.startTime.setMinutes(
        match.startTime.getMinutes() + match.matchDuration,
      );
  ...
  public async checkIfAbleToAssignUmpireToMath(
    matchId: number,
    umpireId: number,
  ): Promise<boolean> {
  ...
  async getMatchDetailsForDivisions(divisionIds: number[]): Promise<Match[]> {
    const matches = await this.entityManager
      .createQueryBuilder(Match, 'm')
