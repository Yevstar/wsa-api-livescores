...
export default class CompetitionOrganisationService extends BaseService<CompetitionOrganisation> {
  ...
  public async softDeleteByOrgId(compOrgId: number, compId: number): Promise<any> {
    ...
    query.where(
      'competitionOrganisation.orgId = :compOrgId and competitionOrganisation.competitionId = :compId',
      { compOrgId, compId },
    );
    return await query.softDelete().execute();
  }

  public async findLinkedCompetitionOrganisation(
    id: number,
  ...
  async getCompetitionParticipatingType(
    competitionId: number,
    organisationId: number,
  ): Promise<CompetitionParticipatingTypeEnum> {
