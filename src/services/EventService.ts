...
export default class EventService extends BaseService<Event> {
  ...
  public async findUserEventOccurrences(userId: number): Promise<any> {
    let result = await this.entityManager.query(
      'call wsa.usp_get_eventOccurrences(?, ?, ?, ?, ?)',
      [userId, Role.MANAGER, Role.PLAYER, EntityType.TEAM, EntityType.USER],
    );
  ...
  public async findUserEventOccurrencesV2(
    ...
    let result = await this.entityManager.query(
      ...
      [userId, eventReceipientsRoleIds.toString(), EntityType.TEAM, EntityType.USER],
    );
    if (isArrayPopulated(result)) {
      return result[0];
    } else {
  ...
  public async createEventOccurrence(
    eventId: number,
    allDay: boolean,
    startTime: Date,
    endTime: Date,
  ...
  public async findEventInvitees(ids: number[]): Promise<any> {
    let result = await this.entityManager.query('call wsa.usp_get_eventInvitees(?, ?, ?)', [
      ids.toString(),
      EntityType.TEAM,
      EntityType.USER,
    ]);
  ...
  private async updateDeletedAtEventAndEventInvitee(
