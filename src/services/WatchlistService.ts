...
export default class WatchlistService extends BaseService<Watchlist> {
  ...
  public async findOrganisationByParam(
    userId: number = undefined,
    deviceId: string = undefined,
    ...
    query.andWhere(
      'linkedCompetitionOrganisation.id in ' +
        this.watchlistSubQuery(query, 'COMPETITION_ORGANISATION', userId, deviceId),
    );
    return query.getMany();
  }

  public async findTeamByParam(
    userId: number = undefined,
    deviceId: string = undefined,
  ): Promise<Team[]> {
  ...
  public async deleteByParam(
    ...
    if (userId) {
      ...
      query.andWhere('deviceId = :deviceId', { deviceId }).andWhere('userId is null');
    }
    ...
    if (userId || deviceId) {
      if (competitionOrganisationIds) {
        for (const id of competitionOrganisationIds) {
          if (
            !currentData.find(
              y => y.entityTypeId == EntityType.COMPETITION_ORGANISATION && y.entityId == id,
            )
          ) {
            records.push(
      ...
      if (teamIds) {
        for (const id of teamIds) {
          if (!currentData.find(y => y.entityTypeId == EntityType.TEAM && y.entityId == id)) {
            records.push(
              WatchlistService.createWatchListItem(deviceId, userId, id, EntityType.TEAM),
            );
          }
        }
      }
  ...
  private static createWatchListItem(
    deviceId: string,
    userId: number,
    entityId,
    type: number,
  ...
  public async copyByUserId(deviceId: string, userId: number): Promise<any> {
    return this.entityManager.query(
      'insert into watchlist (deviceId, entityId, entityTypeId) \n' +
  ...
  public async updateDeviceId(oldDeviceId: string, newDeviceId: string): Promise<any> {
    return await this.entityManager
      .createQueryBuilder(Watchlist, 'wl')
      .update()
