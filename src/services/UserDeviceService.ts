...
export default class UserDeviceService extends BaseService<UserDevice> {
  ...
  public async getUserDevices(userId: number): Promise<UserDevice[]> {
    return this.entityManager
      .createQueryBuilder(UserDevice, 'ud')
    ...
    query.andWhere(
      'ud.userId in ' +
        query
          ...
          .innerJoin('ure.entityType', 'et')
          .andWhere(
            "r.name = 'manager' or r.name = 'player' or r.name = 'umpire' or r.name = 'coach' or r.name = 'spectator'",
          )
          .andWhere(
            new Brackets(qb => {
              qb.orWhere(
    ...
    query.orWhere(
      'ud.userId in ' +
        query
          ...
          .andWhere('r.matchId = :matchId', { matchId: match.id })
          .getQuery(),
    );
    return query.getMany();
  }

  public async findScorerDeviceFromRoster(
    matchId: number,
  ...
  private async findDeviceForUreAndRoster(
    newsId: number,
    ureRoleIds: number[],
    rosterRoleIds: number[],
  ): Promise<any[]> {
    return this.entityManager.query(
  ...
  public async countDistinctDevices(organisationId: number, competitionId: number): Promise<any[]> {
    ...
    if (competitionId == 0) {
      return this.entityManager.query(rawSQL + ';', [EntityType.COMPETITION, organisationId]);
    } else {
      return this.entityManager.query(rawSQL + 'and c.id = ? ' + ';', [
