...
export default class PlayerService extends BaseService<Player> {
  ...
  public async findByParam(
    ...
    if (competition) {
      if (includeLinkedCompetition && competition.linkedCompetitionId) {
        query.andWhere(
          '(team.competitionId = :competitionId OR \n' +
            'team.competitionId = :linkedCompetitionId)',
          { competitionId: competition.id, linkedCompetitionId: competition.linkedCompetitionId },
        );
      } else {
    ...
    if (linkedCompetitionOrganisation) {
      query.andWhere('team.competitionOrganisationId = :compOrgId', {
        compOrgId: linkedCompetitionOrganisation.id,
      });
      if (includeLinkedCompetition) {
        query.andWhere('linkedCompetitionOrganisation.organisationId = :organisationId', {
          organisationId: linkedCompetitionOrganisation.organisationId,
        });
      } else {
        query.andWhere('linkedCompetitionOrganisation.id = :id', {
          id: linkedCompetitionOrganisation.id,
        });
      }
    }
    ...
    if (search) {
      let conditions = [];
      conditions.push(
        `(LOWER(concat_ws(" ", player.firstName, player.lastName)) like :playerName)`,
      );
      conditions.push(
        `(LOWER(concat_ws("", division.divisionName, division.grade)) like :divisionName)`,
      );
  ...
  public async loadPlayersPlayStat(teamId: number): Promise<PlayerMinuteTracking[]> {
    return this.entityManager.query(
      'select pmt.playerId as playerId,\n' +
        '       JSON_OBJECT(\n' +
        "               'id', p.id,\n" +
        "               'userId', p.userId,\n" +
        ...
        'where pmt.teamId = ?\n' +
        'group by pmt.playerId, player',
      [teamId],
    );
  }

  public async loadPlayersPlayStatByMatch(
    matchId: number,
    teamId: number,
  ...
  public async listTeamPlayerActivity(
    ...
    let result = await this.entityManager.query(
      'call wsa.usp_get_team_player_activity(?,?,?,?,?,?,?,?,?,?)',
      [
        competitionId,
        competitionOrganisationId,
        divisionId,
  ...
  public async loadBorrowsForPlayer(playerId: number) {
    return this.entityManager.query(
      'select t.name as teamName, \n' +
        ...
        '       and m.id = gta.matchId',
      [playerId],
    );
  }

  public async getBorrowedPlayersById(playerIds: number[]): Promise<Player[]> {
    let query = this.entityManager
      .createQueryBuilder(Player, 'player')
      .innerJoinAndSelect('player.team', 'team')
      .leftJoinAndSelect('player.user', 'user')
