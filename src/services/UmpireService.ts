...
export class UmpireService extends BaseService<User> {
  ...
  async findManyByCompetitionIdForOrganisation(
    ...
    const isCompetitionOrganizer = await this.competitionService.isCompetitionOrganiser(
      organisationId,
      competitionId,
    );
    let competitionOrganizationsQuery = this.entityManager.createQueryBuilder(
      CompetitionOrganisation,
      'compOrg',
    );

    if (isCompetitionOrganizer) {
      competitionOrganizationsQuery.where('compOrg.competitionId = :competitionId', {
        competitionId,
      });
    } else {
      competitionOrganizationsQuery.where(
        'compOrg.orgId = :orgId AND compOrg.competitionId = :competitionId',
        {
      ...
      switch (sortBy) {
        ...
        case 'rank':
          switch (sortOrder) {
            case 'ASC':
              query
                .addSelect('(IFNULL(competitionRank.rank, 100000))', 'notNullRank')
                .orderBy('notNullRank', sortOrder);
              break;
            case 'DESC':
              query
                .addSelect('(IFNULL(competitionRank.rank, 0))', 'notNullRank')
                .orderBy('notNullRank', sortOrder);
          }
      }
    }
    ...
    if (skipAssignedToPools) {
      const attachedPoolsUmpiresIds = await this.entityManager
        .createQueryBuilder(UmpirePool, 'up')
        .leftJoinAndSelect('up.umpires', 'u')
        .where('up.competitionId = :competitionId', { competitionId })
        .getMany();

      const attachedUmpiresIds = attachedPoolsUmpiresIds.reduce(
        (umpiresIdsAcc: [], pool: UmpirePool) => {
  ...
  async getAllUmpiresAttachedToCompetition(competitionId: number): Promise<User[]> {
    const compOrgs = await this.entityManager
  ...
  getUmpiresQueryAttachedToCompetitionOrganisations(compOrgIds: number[]) {
    return this.entityManager
      ...
      .where(
        'roles.entityTypeId = :entityTypeId AND roles.entityId IN (:compOrgIds) AND roles.roleId IN (:roles)',
        {
  ...
  async findOneByCompetitionId(userId: number, competitionId: number): Promise<User> {
    const umpire = this.entityManager
      .createQueryBuilder(User, 'u')
      .leftJoinAndSelect('u.userRoleEntities', 'roles')
      .leftJoinAndSelect('u.umpireCompetitionRank', 'umpireCompetitionRank')
      .where(
        'roles.entityTypeId = :entityTypeId AND roles.entityId = :entityId AND roles.roleId IN (15,20)',
        {
          entityTypeId: 1,
  ...
  async updateRank(
    ...
    const currentUmpireCompetitionRank = await this.getUmpireRankForCompetition(
      umpireId,
      competitionId,
    );
    ...
    const rankedUmpires = await this.competitionService.getRankedUmpiresCountForCompetition(
      competitionId,
    );
    ...
    if (
      ...
      rank = vacantRank - 1;
      await this.decreaseUmpirePositionWithShiftStrategy(
        competitionId,
        umpireId,
        rank,
        currentUmpireCompetitionRank,
    ...
    if (currentUmpireCompetitionRank && rank > currentUmpireCompetitionRank.rank) {
    ...
    if (rank >= vacantRank) {
      rank = vacantRank;
      const umpireCompetitionRank =
        (await this.entityManager.findOne(UmpireCompetitionRank, {
          umpireId: umpire.id,
    ...
    if (rank < vacantRank) {
      switch (updateRankType) {
        ...
        case 'shift':
          await this.shiftUmpiresRanksList(
            competitionId,
            umpireId,
            rank,
            currentUmpireCompetitionRank,
  ...
  calculateAverageRank(umpire: User): number {
    ...
    return (
      umpire.umpireCompetitionRank
        .map(umpireCompetitionRank => umpireCompetitionRank.rank)
        .reduce((prev, curr) => prev + curr, 0) / umpire.umpireCompetitionRank.length
    );
  }
  ...
  async addOrganisationNameToUmpiresWithURE(umpires: User[]): Promise<User[]> {
    ...
    const compOrgs = await this.competitionOrganisationService.getWithOrganisationByIds(
      uniqueCompOrgIds,
    );

    umpires.forEach(umpire => {
      umpire.organisationName = compOrgs.reduce(
        (orgName: string, currentCompOrg: CompetitionOrganisation) => {
          let currentName;
  ...
  async getUmpireRankAttachedToCompetition(umpireId: number, competitionId: number): Promise<User> {
    const competitionOrganization = await this.entityManager
      .createQueryBuilder(CompetitionOrganisation, 'compOrg')
    ...
    return await this.entityManager
      ...
      .leftJoinAndSelect('u.userRoleEntities', 'roles')
      .where(
        'roles.userId = :umpireId AND roles.entityTypeId = :entityTypeId AND roles.entityId IN (:compOrgIds) AND roles.roleId IN (:roles)',
        {
          entityTypeId: EntityType.COMPETITION_ORGANISATION,
  ...
  async shiftUmpiresRanksList(
    ...
    if (newlyCreatedCurrentCompetitionRank) {
      affectedCompetitionRanks = competitionRanks.filter(
        competitionRank => competitionRank.rank >= rank && competitionRank.umpireId !== umpireId,
      );
    }
  ...
  async replaceUmpiresInRanksList(
    ...
    const competitionRanks = await this.competitionService.getCompetitionRanks(competitionId);
    const umpireRankToBeReplaced = competitionRanks.find(
  ...
  async getUmpireRankForCompetition(
    umpireId: number,
    competitionId: number,
  ): Promise<UmpireCompetitionRank> {
  ...
  async removeUmpireCompetitionRankedFromList(
    umpireId: number,
    competitionId: number,
  ): Promise<void> {
  ...
  async getCompetitionsIdsWhereUmpireRanked(umpireId: number): Promise<number[]> {
    return (
      await this.entityManager
        .createQueryBuilder(UmpireCompetitionRank, 'ucr')
        .where('ucr.umpireId = :umpireId', { umpireId })
        .getMany()
    ).map(umpireCompetitionRank => umpireCompetitionRank.competitionId);
  }
  ...
  async removeUmpireRanksFromCompetitions(umpireId: number): Promise<void> {
    ...
    await Promise.all(
      competitionsIds.map(competitionId =>
        this.removeUmpireCompetitionRankedFromList(umpireId, competitionId),
      ),
    );
  }
  ...
  async getUmpiresTeamsAndOrgRefs(umpiresIds: number[]): Promise<UmpireTeamsAndOrgsRefs[]> {
    ...
    const rawData = await this.entityManager
      .createQueryBuilder(User, 'u')
      .leftJoinAndSelect('u.userRoleEntities', 'ure', 'ure.entityTypeId IN (:entityTypeIds)', {
  ...
  async getUmpiresDivisions(
    competitionId: number,
    umpiresIds: number[],
  ): Promise<UmpireDivisionRef[]> {
    umpiresIds = umpiresIds.length ? umpiresIds : [null];
    const pools = await this.entityManager
      .createQueryBuilder(UmpirePool, 'up')
      .leftJoinAndSelect('up.umpires', 'u')
  ...
  async getDetailedUmpire(
    ...
    const { umpireAllocationSettings } = (await this.umpireSettingsService.getAllocationSettings(
      competitionId,
    ...
    if (isCompetitionOrganizer) {
      allocationSettings = umpireAllocationSettings.filter(setting => {
        return (
          UmpireAllocationTypeEnum.OWN_TEAM === setting.umpireAllocationTypeRefId &&
    ...
    let divisionsIds = _.uniq(
      _.flattenDeep(
        allocationSettings.map(setting => setting.divisions.map(division => division.id)),
      ),
    );
    ...
    selectedTeams = selectedTeams.filter(
      selectedTeam => !teams.every(team => team.id !== selectedTeam.id),
    );
    const selectedOrganisations = _.uniqBy(
      compOrgs.map(compOrg => compOrg.organisation),
      'id',
    );
