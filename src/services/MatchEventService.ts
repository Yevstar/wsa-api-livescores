...
export default class MatchEventService extends BaseService<MatchEvent> {
  ...
  public async findEventsByMatchId(matchId: number, period: number): Promise<MatchEvent[]> {
    let query = this.entityManager
      .createQueryBuilder(MatchEvent, 'matchEvent')
  ...
  public async logMatchEvent(
    ...
    eventTimestamp: Date,
    userId: number,
    attribute1Key: string = undefined,
    attribute1Value: string = undefined,
    attribute2Key: string = undefined,
    attribute2Value: string = undefined,
  ) {
  ...
  public async logLiteMatchEvent(
    matchId: number,
    category: string,
    type: string,
    period: number,
  ...
  public async updateMatchStatEvent(
    ...
    let query = this.entityManager
      .createQueryBuilder(MatchEvent, 'me')
      .update(MatchEvent)
  ...
  public async findByParams(
    ...
    if (this.isGameStatGoalOrPoints(gameStatCode)) {
      query.andWhere(
        '(matchEvent.eventCategory = :scoreEventCategory or ' +
      ...
      if (isNotNullAndUndefined(recordAssistPlayer) && recordAssistPlayer) {
        query.andWhere(
          '(matchEvent.type = :scoreEventType or ' +
        ...
        query.andWhere(
          '(matchEvent.type = :scoreEventType or ' + 'matchEvent.type = :statEventType)',
          {
            scoreEventType: 'update',
            statEventType: gameStatCode,
          },
      ...
      if (teamSequence == 1) {
        query.andWhere(
          '(matchEvent.attribute1Key = :scoreAttribute1Key ' +
      ...
      if (isNotNullAndUndefined(recordAssistPlayer) && recordAssistPlayer) {
        query.andWhere(
          '(matchEvent.attribute1Value = :scoreAttribute1Value ' +
            ...
            'or matchEvent.attribute1Value = :assistAttribute1Value)',
          {
            ...
            statAttribute1Value: recordPoints
              ? points.toString()
              : positionId
              ? positionId.toString()
              : '',
      ...
      query.andWhere(
        '(matchEvent.attribute2Key = :scoreAttribute2Key ' +
      ...
      if (isNotNullAndUndefined(recordAssistPlayer) && recordAssistPlayer) {
        query.andWhere(
          '(matchEvent.attribute2Value = :scoreAttribute2Value ' +
      ...
      query
        .andWhere('matchEvent.attribute1Value = :attribute1Value', {
          attribute1Value:
            recordPoints || (gameStatCode == GameStatCodeEnum.F && isNotNullAndUndefined(foul))
